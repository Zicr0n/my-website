...
for(let index in data.results){
    pokemonList.push(data.results[index])
}

export function getPokemonByLetters(string)
{
    let listUnsorted = []
    let listSorted = []
    ...
    for(let index in pokemonList){
        if(pokemonList[index].name.includes(string) || data.results[index].url.split("/")[6].includes(string)){
            // FETCH ID based off of URL
            let id = data.results[index].url.split("/")[6]
            let name = pokemonList[index].name
            name = name.charAt(0).toUpperCase() + name.slice(1)
            
            listUnsorted.push({name : name, id : id})
        }
    }

    // Now sorted list is defined as the unsorted list after its sorting by id, but only names are returned
    listSorted = listUnsorted.sort((a, b) => a.id - b.id).map((element) => element.name)
